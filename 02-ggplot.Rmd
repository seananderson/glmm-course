...
Experiment with ggplot2 geoms. Try applying some different geoms to the `morph` dataset. For example, try showing the distribution of wing length with `geom_histogram()`:

```{r}
ggplot(morph, aes(wingl)) + geom_histogram() # exercise
```

You could also try showing the distribution of wing length for male and female birds by using `geom_violin()` (hint: `sex` will be the `x` aesthetic):

```{r}
ggplot(morph, aes(sex, wingl)) + geom_violin() # exercise
```

Try making a boxplot with `taxon` on the x axis and `wingl` on the y axis.

```{r}
ggplot(morph, aes(taxon, wingl)) + geom_boxplot() # exercise
```
