---
Title: Manipulating data with dplyr and pipes
---

# Goals

- Become familiar with the 5 dplyr verbs of data manipulation
- Be comfortable chaining multiple R commands with the `%>%` (pipe) symbol
...
download.file(pantheria, destfile = "data/generated/mammals.txt")
```

We'll load the dplyr and readr packages:
...
```{r}
mammals <- read_tsv("data/generated/mammals.txt")
...
Your turn: filter `mammals` for all rows where `adult_body_mass_g` is `NA` and `adult_head_body_len_mm` is greater than 2000. Hint (use `is.na()`):

```{r}
filter(mammals, is.na(home_range_km2), adult_head_body_len_mm > 2000)
```
...
Your turn now. Take what I started and add a line that keeps only the rows where
`median_litter` is less than 3:

```{r}
mammals %>% group_by(order) %>%
  summarise(median_litter = median(litter_size, na.rm = TRUE)) # <- add a %>% here
  # <- add a line of code here with the `filter()` function
```

*Extension*: for bonus points, arrange the data frame by `median_litter` and select only the columns `order` and `median_litter` by adding two more lines.

Again, your turn: make the following piece of code easier to read by converting it to use pipes:
...
summarise(
  group_by(
    ...
    order), 
  mean_body_mass = mean(adult_body_mass_g))
```
