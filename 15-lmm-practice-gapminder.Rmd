...
Try plotting the model predictions. Let's focus on the effect of GDP per capita. Therefore, we will have to set the predictor `decade` to some value. Let's set it to `0`, which represents 1990 because of how we calculated this column by  subtracting 1990.
...
Let's add a group-level predictor representing the mean log GDP per capita for each country. At the same time, let's center the GDP per capita within each country. Why do we do this again?
...
The final plot we made works because because we only have one predictor (GDP per capita). How might we make a similar plot if we had multiple predictors?
