...
If our zero inflation parameter is `pz`, then our model will that with probability 1-pz, the response data comes from a Poisson (or negative binomial) distribution, and with probability pz, the data is zero.
...
In the next plot, the open circles are access zeros and the closed circles are the ones derived from the Poisson.
...
The glmmADMB package lets you fit zero-inflated GLMMs, but we are going to use glmmTMB, which is newer, usually faster, and less likely to cause strange errors. 
...
glmmTMB returns the zero inflation parameter on the logit scale. But we can back transform that with the inverse logit transformation `plogis`. Remember, the true value was 0.2.
...
Let's fit 2 separate models. First a truncated Poisson to the positive values. This is a Poisson distribution that starts at one instead zero.
...
We can derive the prediction of the hurdle model by multiplying the probability predicted by the binomial model with the production from the positive-value model:
