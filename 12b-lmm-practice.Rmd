...
- What does slope on the group-level predictor `mean_log_gdp_percap_cent` tell us? 
...
Our last plot was allowing for GDP per capita to vary both within and across countries. What if we just wanted to show the across country effect? One way to do that would be to set the within country GDP per capita variable to its mean value, 0:

```{r}
newdata <- mutate(gap, decade = 0, log_gdp_percap_cent = 0)
newdata$predict3 <- predict(m3, newdata = newdata)
ggplot(newdata, aes(mean_log_gdp_percap_cent, predict3, 
...
```{r}
sjPlot::sjp.lmer(m3, type = "fe", p.kr = FALSE, show.p = FALSE) # exercise
```
...
sjPlot::sjp.lmer(m3, type = "re") # exercise
```
