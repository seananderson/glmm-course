---
Title: R formula and random effect grouping syntax
---
...
What are 2 ways to remove the intercept in an R formula? Hint: read `?formula`.
...
A group of grad students are trying to model log(frog density) as a function of soil characteristics. What might their model look like in the following cases (using `lme4::lmer`)? Give cases with random intercepts only and cases with random intercepts and slopes.

Jerry measures frog density and soil characteristics from 1 transect (`transect`) within 6 separate ponds (`pond`).

```{r}
log(frog_dens) ~ soil + 
  (1 | pond) # exercise
log(frog_dens) ~ soil + 
  (1 + soil | pond) # exercise
```
...
```{r}
log(frog_dens) ~ soil + (1 + soil | transect) # exercise
log(frog_dens) ~ soil + (soil | transect) # exercise
```
...
```{r}
log(frog_dens) ~ soil + (0 + soil | transect) # exercise
```

## Your turn 2

Jane measures frog density and soil characteristics from 2 transects (`transect`) within 4 ponds (`pond`). Assume her data are structured like this:
...
Ignoring that Jane probably hasn't sampled enough transects to get much benefit from a mixed effect model:
...
```{r}
ranef(m_wrong)$transect # exercise
ranef(m3)$transect # exercise
```
...
```{r}
(m_nlme <- nlme::lme(log(frog_dens) ~ soil, 
