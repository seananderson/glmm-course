...
What does the coefficient `fixef(m_slopes_gr)[["mean_log_wingl_centered"]]` represent? How might we make this coefficient more interpretable?

# Group-level predictor centering

Another option is to center the predictor with in each group. This separates the across-group effect from the within group effect of the wing length predictor. There it was a good recent discussion on the concept in the literature ending with this paper: Bell and Fairbrother 2017 <https://doi.org/10.1007/s11135-017-0593-5>

To do this we simply subtract the taxon-level mean from the wing length predictor within each taxon:

```{r}
d <- group_by(d, taxon) %>% 
  mutate(log_wingl_group_centered = log(wingl) - mean(log(wingl)))
```

And refit our model with the new group-level centered predictor:

```{r}
m_slopes_gr_cent <- lmer(log(beakh) ~ log_wingl_group_centered + mean_log_wingl_centered +
    (1 + log_wingl_group_centered | taxon), data = d)
```
...
d$predict_slopes_gr_cent <- predict(m_slopes_gr_cent)

across_group_int <- fixef(m_slopes_gr)[["(Intercept)"]]
across_group_slope <- fixef(m_slopes_gr)[["mean_log_wingl_centered"]]
across_group_int <- fixef(m_slopes_gr_cent)[["(Intercept)"]]
across_group_slope_centered <- fixef(m_slopes_gr_cent)[["mean_log_wingl_centered"]]

ggplot(d, aes(log_wingl_centered, log(beakh), colour = taxon, group = taxon)) +
  geom_point(alpha = 0.1) +
  geom_line(aes(y = predict_slopes_gr_cent), colour = "black") +
  geom_line(aes(y = predict_slopes_gr), colour = "red") +
  geom_abline(intercept = across_group_int, slope = across_group_slope, colour = "black", lwd = 2) +
  geom_abline(intercept = across_group_int, slope = across_group_slope_centered, colour = "red", lwd = 2)
```

What's the most striking difference between the black lines (no group-level centering) and the red lines (group-level centering)?

Which do you find most interpretable?
...
ggplot(d, aes(log_wingl_centered, log(beakh), colour = taxon, group = taxon)) +
  geom_point(alpha = 0.1) +
  geom_line(aes(y = predict_slopes_gr_cent, size = n), colour = "black") +
  geom_smooth(method = "lm", se = FALSE)
```
...
filter(d, taxon %in% c("Geospiza scandens", "Geospiza scandens scandens")) %>%
  ...
  geom_line(aes(y = predict_slopes_gr_cent), colour = "black", lwd = 1.5)
```
...
estimates <- d %>% group_by(taxon) %>%
  ...
  select(-mod)
estimates$re_int <- coef(m_slopes_gr_cent)$taxon[,1]
estimates$re_slope <- coef(m_slopes_gr_cent)$taxon[,2]
...
Bonus question: in a typical shrinkage plot the estimates are all pulled towards the mean. That isn't always the case here (see the crossing lines above). Why is that happening with this model?
