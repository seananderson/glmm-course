---
Title: Introduction to random intercepts
---

# Goals

- Learn to fit basic random intercept models with the `lme4::lmer` function
...
morph <- morph %>%
  select(islandid, taxonorig, genusl69, speciesl69, sex, wingl, beakh, ubeakl) %>%
  rename(taxon = taxonorig, genus = genusl69, species = speciesl69)
morph <- data.frame(na.omit(morph)) # remove all rows with any NAs to make this simple
morph <- filter(morph, genus == "Geospiza") %>% as_data_frame()
d <- morph
```
...
We are going to look at predicting the length of birds' beaks based on their measured wing length. And we have measurements for a variety of species. 

```{r}
ggplot(d, aes(wingl, beakh, colour = taxon)) + geom_point()
```

We could fit a model to these data as they are plotted here. But we might want to consider transforming our predict our variable (wing length) and/or our response variable (beak length). What might that transformation be, and why would we want to do that? 
...
One option would be to fit a single linear regression to these data. We can do that right from within ggplot2. Let's look at what that would look like:
...
The workhorse function of the lme4 package for fitting mixed effects models is the function `lmer`. You can pronounce it however you like. Some people would say "lemur". The syntax is just like `lm` with the addition of a formula for the "random effects".

Let's fit a random intercept model now:

```{r}
m_lmer <-  lmer(log(beakh) ~ log(wingl) + (1 | taxon), data = d)
```

Notice the new piece `+ (1 | taxon)`. The `1` it refers to "intercept" and the `| taxon` tells the function that you want to let the intercept very by the values in the `taxon` column.

# Plotting the predictions

Let's look at what we just did. Will start by plotting the predictions. To do that, will use the `predict` function. Because we are applying it an object of class `merMod` (meaning it was created by the `lmer` function), we can get details on that function at `?predict.merMod`.
...
There is another kind of prediction we can make with a mixed effects model. We can make a prediction at the "population" level. This is our expectation if we sampled a new taxon of unknown identity:

```{r}
d$predict_lmer_population <- predict(m_lmer, re.form = NA)
```

Let's add the population prediction to the plot:
...
This tells us the variance and standard deviation (Variance = sqrt(Std.Dev.)) of our random intercept on `taxon` and of the residual variation `Residual`. We are also told how many rows of data we have `Number of obs: 1434` and how many random intercept groups we have `taxon, 15`.
...
This tells us that our intercept and slope are highly correlated. This isn't a great, and if this correlation is very strong and our model is complicated it might result in computational problems. Later we will talk about ways of solving this (hint: we'll "center" the predictor). 

There are a lot of details in the output from `summary.merMod` and usually a lot more decimal points than are necessary. 

A nice alternative is the function `arm::display` which focuses on the important pieces. 

```{r}
arm::display(m_lmer)
```

We will tend to use this function throughout the workshop. I tend not to load the package arm, and instead call its functions directly (e.g. `arm::display()`) because it loads many other functions and packages.

# Interpreting the model predictions

What do the model predictions mean?

For the population level estimate (`fixef(m_lmer)`), our slope is:
...
What does our intercept mean? Will in this case, it's not very meaningful. It's the expected log(beak length) when log(wing length) = 0. In other words, we would expect a bird with a wing length of 1 to have a beak length of `round(exp(fixef(m_lmer)[[1]]), 2)`.
...
- we've assumed each taxon is independent but some are obviously more phylogenetically related to each other than others <!-- exercise -->
...
- we might want to try centering the predictor
