...
p <- sjp.glm(m_gamma, type = "eff", show.ci = TRUE, prnt.plot = FALSE)
...
# or:
# p$plot + geom_point(data = d, aes(x = x, y = y), inherit.aes = FALSE)
```
...
p <- sjp.glm(m_nb, type = "eff", show.ci = TRUE, prnt.plot = FALSE)
ggplot(p$data, aes(x, y, ymin = conf.low, ymax = conf.high)) +
  geom_ribbon(size = 1, alpha = 0.2) + geom_line(colour = "red") + 
  geom_point(data = d, aes(x = x, y = y), inherit.aes = FALSE)
```
...
p <- sjp.glm(m_bin, type = "eff", show.ci = TRUE, prnt.plot = FALSE)
g <- ggplot(p$data, aes(x, y, ymin = conf.low, ymax = conf.high)) +
  geom_ribbon(size = 1, alpha = 0.2) + geom_line(colour = "red") + 
  geom_point(data = d, aes(x = x, y = y), inherit.aes = FALSE)
print(g)
