...
```{r}
plot_glm <- function(model, dat, x = "x", y = "y") {
  ...
  p <- ggplot(dat) + geom_point(aes_string(x, y), alpha = 0.6) + 
    geom_line(aes_string(x, "fit"), colour = "red", lwd = 1) +
    geom_ribbon(aes_string(x, ymax = "upr", ymin = "lwr"), alpha = 0.2)
  print(p)
  invisible(p)
}
```
