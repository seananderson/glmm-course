...
d <- select(d, survived, age, fare, female)
d
```
...
Now we're going to refit the above model with versions of the predictors that have been scaled (by dividing by 2 standard deviations) and centered (by dividing their mean value). For the binary predictor (`female`) the variable will be centered by its mean value but not scaled. This standardizing procedure will make the magnitude of the coefficients approximately comparable. 

```{r}
d$age_scaled <- arm::rescale(d$age)
d$fare_scaled <- arm::rescale(d$fare)
d$female_centered <- arm::rescale(d$female)
# or d$female_centered <- d$female - mean(d$female)
...
Now refit the model with all 2-way interactions but with the scaled versions of age and fare: `age_scaled` add `fare_scaled`. This time use the 0-1 version of `female`:
...
If a man on the Titanic had paid \$250 for his ticket, what are his odds of surviving compared to another man who paid \$150? (Note that the predictors are in terms of 2 standard deviations of the raw variable. And 2 standard deviations of `fare` is about \$100). I.e.:
...
And what would be the odds ratio for 2 women's chances of survival if one had paid an extra \$100?

```{r}
exp(coef(m3)[["fare_scaled"]] + coef(m3)[["fare_scaled:female"]] * 1) # exercise
exp(0.89 + 1.31 * 1) # exercise
```
...
Why is the effect of `fare_scaled` different from the effect of `fare` (and similarly for the effect of age)? What do these 2 coefficients represent? 
...
How would you derive the same odds ratios as above? (The effect of paying an extra \$100 for a ticket for a man and for women?) Remember that these models make the same predictions, they are just parameterized slightly differently.

```{r}
exp(coef(m4)[["fare_scaled"]] + coef(m3)[["fare_scaled:female"]] * 0.65) # exercise
exp(1.35 + 1.31 * 0.65) # exercise

exp(coef(m4)[["fare_scaled"]] + coef(m3)[["fare_scaled:female"]] * -0.35) # exercise
exp(1.35 + 1.31 * -0.35) # exercise
```
...
If you made it this far, read in the original data frame again and try plotting and modeling some of the other predictors. Are there other interesting patterns to be found? If you need more documentation on the data, have a look at <https://www.kaggle.com/c/titanic/data>.
