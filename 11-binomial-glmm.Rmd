...
- Become familiar with building, checking, and plotting a binomial GLMM
...
Indeed the data support including both of these interactions with time and this would likely be an important scientific question from the experiment. 
...
comparison_effect <- function(model, base_par, comparison_par) {
  if (class(model) == "glmmTMB") {
    fe <- fixef(model)$cond
    se <- coef(summary(model))$cond[,"Std. Error"]
  } else {
...
summary(m4)
comparison_effect(m4, "drying", "drying")
comparison_effect(m4, "drying", "speciesc:drying")
comparison_effect(m4, "drying", "speciesb:drying")

comparison_effect(m4, "warming", "speciesb:warming")
```
