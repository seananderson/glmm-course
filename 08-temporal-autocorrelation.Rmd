...
m1 <- lmer(log(depth_mean_day) ~ temperature_centered + 
    (1 + temperature_centered | fish), data = d)
arm::display(m1)
```
...
Another useful plot to look at is an "autocorrelation function" plot (ACF). This calculates the correlation at increasingly large lags for the residuals within each group (cod fish). We will be using the function `ACF.lme` Again, we will specify `resType = "normalized"`:
...
ggplot(newdata) + 
  ...
  ylab("Depth (m)") + xlab("Temperature (C)") +
  theme_light() +
