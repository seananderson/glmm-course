...
air <- as_data_frame(air) %>% select(-ozone, -solar.r, -wind)
```
...
Make the following dataset (stock prices for stocks X, Y, and Z) long with columns for time, stock ID, and price. Save the output into `stocks_long`.
...
```{r}
stocks_long <- stocks %>% gather(key = stock, value = price, -time)
stocks_long
```
...
```{r}
stocks_long %>% spread(key = stock, value = price)
```

*Extension*: There's another possible wide format for this dataset. How could you use `spread` to have a column for each time and a row for each stock?

Answer:

```{r}
stocks_long %>% spread(key = time, value = price)
```

# Further information

Parts of the following exercise are modified from:
