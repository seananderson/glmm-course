...
There are lots of different ways we could extract the coefficients from each element of the list. Here we will use one of the `map` functions from Hadley Wickham's purrr package. If you are familiar with the `apply` functions in R, the `map` functions are simply a consistent easy to work with version of the `apply` functions.
...
We can use the sjPlot package to make some quick plots. Take a look at the help for `?sjp.glmer` and try out the various plots that are available through the `type` argument:

```{r}
library(sjPlot)
sjp.glmer(m) # exercise
sjp.glmer(m, type = "re.qq")  # exercise
sjp.glmer(m, type = "fe")  # exercise
p <- sjp.glmer(m, type = "eff", show.ci = TRUE)  # exercise
head(p$data) # exercise
```

The sjPlot package has automatically exponentiated estimates so these are multiplicative effects on the odds of metamorphosing. 
