...
We are going to work with a data set from [O'Regan et al. 2013](http://doi.org/10.1890/13-0916.1.
...
These data represent time to metamorphosis for 3 amphibian species in a mesocosm experiment. The experiment had 2 levels of warming (normal and accelerated) and 2 levels of pond drying (normal/permanent and accelerated/temporary). There were multiple tanks per treatment condition.
...
We are going to model the expected probability of a randomly chosen amphibian
having reached metamorphosis at any given point in time and for each of the
treatments.

In other words, `meta` (1 = metamorphosis) will be our response, and the
predictors will be `week_centered`, `warming`, `drying`, and `species`. I've
also included columns for `treatment` (warming and drying combination) and
`species_true` to make plotting easier. 
...
A good first step is to fit a global model with no random effects and separate
models for each potential random effect level.
...
How do these plots help build our intuition about what we expect to find when
we fit more complicated GLMMs?
...
Default residual plots are not particularly useful for a binomial response,
although adding a smooth function helps a little bit. 

```{r}
plot(m, type = c("p", "smooth"), ylim = c(-3, 3))
```

Looking at the residuals by group can also be useful. What are we looking for
here? 
...
One useful way of checking residuals for a binomial response is by using
a binned residual plot. Gelman and Hill (2007) cover this type of plot in their
textbook on multilevel modeling. 

The idea is to group the residuals into bins based on the predictor values. The
trick is picking an appropriate number of bins for this to be useful. We can
make all the same types of plots with these binned residuals. 
...
This does not look great, but this was just a starting point. We will be improving this model below. 
...
We can access Wald various confidence intervals with:
...
The profile confidence intervals do not assume a quadratic log-likelihood surface but ignore "finite-size" issues. (They assume a very large number of groups, e.g. > 50.)

The bootstrap confidence intervals are the most trustworthy but will take a long time to generate.
...
The sjPlot package has automatically exponentiated the estimates so these are multiplicative effects on the odds of metamorphosing. 

# Model comparison 

We can compare our model without a predictor, say species, via AIC or likelihood ratio test:
...
We might want to consider letting some of these effects vary by tank. This
model will not fit with lme4. Instead we will rely on TMB.

If I was publishing this analysis I would want to be extra careful that the
model had converged given that it failed with default settings in lme4. I might
play with the control settings for lme4 or try fitting the model with glmmADMB
or `MASS::glmmPQL`. 
...
We would probably also want to consider if the effect of time varies by
treatment:
...
This looks better, although not perfect. It's possible that
a different link function would fit the data better although none of the
obvious alternatives looked much better to me. For example:
